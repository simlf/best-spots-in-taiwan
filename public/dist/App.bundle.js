...
function ajaxHeart(e) {
  ...
  _axios2.default.post(this.action).then(function (response) {
    ...
    if (isHearted) {
      _this.heart.classList.add('heart__button--float'); // add a CSS animation to the hearts
      setTimeout(function () {
        return _this.heart.classList.remove('heart__button--float');
      }, 2500); // after 2.5 seconds, remove this animation
    }
  }).catch(console.error);
}
...
var _slicedToArray = function () { function sliceIterator(arr, i) { var _arr = []; var _n = true; var _d = false; var _e = undefined; try { for (var _i = arr[Symbol.iterator](), _s; !(_n = (_s = _i.next()).done); _n = true) { _arr.push(_s.value); if (i && _arr.length === i) break; } } catch (err) { _d = true; _e = err; } finally { try { if (!_n && _i["return"]) _i["return"](); } finally { if (_d) throw _e; } } return _arr; } return function (arr, i) { if (Array.isArray(arr)) { return arr; } else if (Symbol.iterator in Object(arr)) { return sliceIterator(arr, i); } else { throw new TypeError("Invalid attempt to destructure non-iterable instance"); } }; }(); // library for XMLHTTP and http requests
...
// sanitizes the data (in this case it will prevent the user for submitting HTML in a form)

function searchResultsHTML(spots) {
  return spots.map(function (spot) {
    return '\n      <a href="/spot/' + spot.slug + '" class="search__result">\n        <strong>' + spot.name + '</strong>\n      </a>\n    ';
  }).join('');
} // library for XMLHTTP and http requests
...
function typeAhead(search) {
  ...
  searchInput.on('input', function () {
    ...
    _axios2.default.get('/api/search?q=' + this.value).then(function (response) {
